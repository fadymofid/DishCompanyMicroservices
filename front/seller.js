document.addEventListener('DOMContentLoaded', function() {
    ...
    function displaySoldDishes(soldDishes) {
        soldDishesList.innerHTML = '';
        if (!soldDishes || soldDishes.length === 0) {
            soldDishesList.innerHTML = '<li class="list-group-item">No sold dishes found.</li>';
            return;
        }
        // Only show each dish once (by dishId)
        const uniqueDishes = [];
        const seen = new Set();
        for (const soldDish of soldDishes) {
            if (!seen.has(soldDish.dishId)) {
                uniqueDishes.push(soldDish);
                seen.add(soldDish.dishId);
            }
        }
        uniqueDishes.forEach(soldDish => {
            const li = document.createElement('li');
            li.className = 'list-group-item';
            li.innerHTML = `
                <strong>Dish Name:</strong> ${soldDish.dishName} <br>
                <strong>Description:</strong> ${soldDish.dishDescription} <br>
                <strong>Price:</strong> $${soldDish.dishPrice} <br>
                <button class="btn btn-info btn-sm view-buyers-btn" data-dishid="${soldDish.dishId}" data-dishname="${soldDish.dishName}">View Buying Customers</button>
            `;
            soldDishesList.appendChild(li);
        });
        setTimeout(() => {
            soldDishesList.querySelectorAll('.view-buyers-btn').forEach(btn => {
                btn.addEventListener('click', async function() {
                    const dishId = this.getAttribute('data-dishid');
                    const dishName = this.getAttribute('data-dishname');
                    await showBuyingCustomersModal(dishId, dishName);
                    ...
                    if (item.dishId == dishId) {
                        ...
                        try {
                            ...
                            if (shippingResp.ok) {
                                ...
                                if (shippingOrder.company && shippingOrder.company.name) {
